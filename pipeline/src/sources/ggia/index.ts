...
const SOURCE_KEY = "ggia"
const BASE_DOMAIN = "https://ggia.berkeley.edu"
const SEED_URLS = [`${BASE_DOMAIN}/practices`]
...
const DETAIL_PAGE_PATTERN = /^\/practices\/[a-z0-9-]+\/?$/i
const LISTING_PAGE_PATTERN = /^\/practices(?:\/|$)/i
...
const extractMetaContent = (html: string, key: string): string | null => {
  ...
  for (const pattern of patterns) {
    ...
    const content = htmlToText(match?.[1] ?? "").replace(/\s+/g, " ").trim()
...
const discover = async (ctx: SourceModuleContext): Promise<DiscoveredPage[]> => {
  ...
  while (
    ...
    try {
      ...
      ctx.logger.warn("GGIA discovery fetch failed", {
        url: currentUrl,
...
const extract = async (
  ...
  if (classification === "detail") {
    ...
    const h1Title = extractTagText(html, "h1").at(0) ?? titleFromSlug(pageUrl) ?? "Practice gratitude"
    const description = extractMetaContent(html, "description") ?? extractTagText(html, "p").at(0) ?? h1Title
    return [
      {
        sourceKey: SOURCE_KEY,
        sourceUrl: pageUrl,
        ...
        meta: {
          ...
          source_evidence: {
            document_region: "h1_or_meta_description",
            detail_url: pageUrl,
          },
          extractor_version: "ggia_v1",
        },
      },
    ]
  }
...
export const createGgiaSource = (): SourceModule => ({
  key: SOURCE_KEY,
  displayName: "Greater Good In Action",
  discover,
  extract,
  healthCheck,
})
